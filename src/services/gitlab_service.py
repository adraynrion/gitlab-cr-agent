...
class GitLabService:
    ...
    async def get_merge_request(
        ...
        try:
            ...
            logger.error(f"GitLab API error {e.response.status_code} for MR {mr_iid}: {e.response.text}")
            raise GitLabAPIException(
    ...
    async def get_merge_request_diff(
        ...
        try:
            ...
            logger.error(f"GitLab API error {e.response.status_code} for MR diff {mr_iid}: {e.response.text}")
            raise GitLabAPIException(
    ...
    async def post_merge_request_comment(
        ...
        try:
            ...
            raise GitLabAPIException(
                ...
                details={"project_id": project_id, "mr_iid": mr_iid, "comment_length": len(comment)},
                original_error=e
            )
        except httpx.HTTPStatusError as e:
            logger.error(f"GitLab API error {e.response.status_code} posting comment to MR {mr_iid}: {e.response.text}")
            raise GitLabAPIException(
